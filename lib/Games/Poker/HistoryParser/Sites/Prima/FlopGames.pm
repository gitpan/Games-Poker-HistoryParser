package Games::Poker::HistoryParser::Sites::Prima::FlopGames;
...
sub _get_action{
    ...
    my ( $starting, $preflop, $flop, $turn, $river );
    ...
    if( $flop ){
        ...
        # Format cards from "4 of Hearts" to "4h"
        my @cards;
        foreach( split /,/, $cards ){
            my ( $rank, $suit ) = _format_card( $_ );
            push @cards, $rank . $suit;
        }

          $metadata->{'board'}{'flop'}  = join ',', @cards;
        $metadata->{'action'}{'flop'} = $flop;
       }

       if( $turn ){
        my ( $card, $turn ) = split /\n/, $turn, 2;    
           $metadata->{'board'}{'turn'}  = join '', _format_card( $card );;
        $metadata->{'action'}{'turn'} = $turn;
    }

    if( $river ){
        my ( $card, $river ) = split /\n/, $river, 2;    
           $metadata->{'board'}{'river'}  = join '', _format_card( $card );;
...
Sites::Prima::FlopGames - Parsing of flop games such as Hold'em and Omaha

=head1 SYNOPSIS

 use Sites::Prima::FlopGames;
